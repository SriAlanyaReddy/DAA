...
class Solution {
    ...
    private static int decodeWays(String s, int index) {
        // Base case: If we've reached the end of the string, there's 1 way to decode (an empty string)
        if (index == s.length()) {
            return 1;
        }
        ...
        // Decode considering the current single digit
        int result = decodeWays(s, index + 1);

        // Decode considering the current two digits if valid
        if (index < s.length() - 1) {
